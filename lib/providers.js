...
const PROVIDERS = {
  groq: {
    ...
    supportsVision: true, // only the Llama 4 Scout model
    // Llama 4 Scout is Groq's free multimodal model (up to 5 images, 4MB base64).
    // Screenshots auto-route here; the text models can't take image content.
    vision: {
    ...
    instructions:
      "Sign in to console.groq.com, open API Keys, create a new key, and paste it below. Groq has a generous free tier with very fast inference. Screenshots automatically use the free vision model 'meta-llama/llama-4-scout-17b-16e-instruct'."
  },
  openrouter: {
    id: "openrouter",
    ...
    supportsVision: true,
    vision: {
      // current free vision-capable models (verified against the live OpenRouter
      // model list); screenshots auto-route to the fallback when the chosen model
      // is text-only.
      models: [
        "google/gemma-4-31b-it:free",
        "moonshotai/kimi-k2.6:free",
    ...
    instructions:
      "Create an account at openrouter.ai, go to Keys, generate one and paste below. Any model ending in ':free' costs nothing — default is NVIDIA Nemotron 3 Super (free, 1M context). Screenshots auto-use a free vision model (Gemma 4 / Kimi)."
  },
  gemini: {
    id: "gemini",
    ...
    supportsVision: true,
    vision: { models: null }, // every Gemini model reads images
  ...
  anthropic: {
    ...
    supportsVision: true,
    vision: { models: null }, // all Claude 4.x models read images
  ...
  perplexity: {
    ...
    vision: null, // text only — screenshots are rejected with a clear error
...
function buildRequest(providerId, apiKey, payload) {
  ...
  // Sending a screenshot? Make sure we hit a model that can actually read it.
  // Text-only models reject array content ("messages[0].content must be a
  // string"), so we auto-route to the provider's vision model — or, if the
  // provider can't see images at all, fail with a message that tells the user
  // exactly what to switch to.
  if (hasImage) {
    const v = provider.vision;
    if (!v) {
      throw new Error(
        provider.label +
          " can't read images. Open Smartii settings and pick a vision provider — " +
          "Google Gemini (free) or Groq's free Llama 4 Scout both work."
      );
    }
    if (Array.isArray(v.models) && !v.models.includes(model)) {
      model = v.fallback || v.models[0];
    }
  }
